...
using static EorzeanScribe.Gui.MessageBox;

namespace EorzeanScribe;

internal static class EorzeanScribeUI
{
    ...
    private static WindowSystem _windowSystem = new("EorzeanScribe");
    ...
    internal static void Dispose()
    {
        try
        {
            // Use WindowSystem's built-in removal method first
        ...
        {
            EorzeanScribe.PluginLog.Error($"Error during UI disposal: {e.Message}");
        }
    }
    ...
    internal static void Draw()
    {
        ...
        // the draw function.
        bool bResetConfigSaved = EorzeanScribe.Configuration.RecentlySaved;
        try
        {
        ...
        catch ( Exception e )
        {
            ...
            // Log the error.
            EorzeanScribe.PluginLog.Error( $"There was an exception in the WindowSystem draw process. All windows have been hidden for now.\nError: {e}\nMessage: {e.Message}" );

            // Attempt to add an error window to be displayed. If it fails to draw we'll end up back here anyway.
            if ( !Contains("EorzeanScribeUI Error") )
        ...
        finally
        {
            ...
            if ( bResetConfigSaved )
                EorzeanScribe.Configuration.RecentlySaved = false;
        }
    }
    ...
    internal static void RemoveWindow( Window w )
    {
        ...
        if ( !_window_lock )
        {
            ...
            {
                EorzeanScribe.PluginLog.Error( $"ERROR: {e.Message}\n{e}" );
            }
        }
...
#region Alerts and Messages
    ...
    internal static void ShowScratchPad( string str ) => ShowScratchPad();
