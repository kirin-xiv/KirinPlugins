...
public enum MarkerPosition { BeforeBody, AfterBody, AfterContinuationMarker }
public enum RepeatMode { All, AllExceptFirst, AllExceptLast, OnlyOnFirst, OnlyOnLast, EveryNth }

[Flags]
public enum DisplayMode { WithMultipleChunks = 1, WithSingleChunk = 2, WithAnyChunkCount = 3, Always = 3}
...
public sealed class ChunkMarker
{
    ...
    private MarkerPosition _position = MarkerPosition.AfterBody;
    ...
    private DisplayMode _displayMode = DisplayMode.WithMultipleChunks;
    ...
    internal bool Visible(int chunkCount)
    {
...
internal sealed class PadState
{
    ...
    public override int GetHashCode() => HashCode.Combine( this.Header?.ChatType, this.ScratchText, this.Header?.TellTarget );

    public override string ToString() => $"{{ ChatType: {this.Header?.ChatType}, ScratchText: \"{this.ScratchText}\", TellTarget: \"{this.Header?.TellTarget ?? ""}\", CrossWorld: {this.Header?.CrossWorld}, Linkshell: {this.Header?.Linkshell} }}";
}
...
internal sealed class Word
{
    ...
    /// <summary>
    /// Indicates whether suggestions are currently being generated.
    /// </summary>
    internal bool GeneratingSuggestions;
    ...
    internal void GenerateSuggestions(string wordText)
    {
        ...
        if (this.GeneratingSuggestions) return;
        
        this.GeneratingSuggestions = true;
        this.Suggestions = null; // Keep as null while generating
