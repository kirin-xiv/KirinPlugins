...
internal class StatisticsTracker
{
    ...
    internal void TallyWord( string line, Word word)
    {
        try
        {
            ...
            {
                EorzeanScribe.PluginLog.Verbose( $"Word has a length of zero. ([{word.StartIndex}..{word.EndIndex}] {line[word.StartIndex..word.EndIndex]}\")" );
                return;
            }
        ...
        {
            EorzeanScribe.PluginLog.Error( $"Failed to Tally Word \"[{word.StartIndex}..{word.EndIndex}]{line[word.StartIndex..word.EndIndex]}\"" );
            return;
        }
    }
