...
internal sealed class ChatHelper
{
    ...
    internal static List<TextChunk>? FFXIVify(HeaderData header, string text)
    {
        UTF8Encoding encoder = new();

        // Get the number of bytes taken by the header.
        // We then cut the bytes out required for the safety zone, header, and continuation marker.
        int iMaxByteWidth = 480 - SAFETY - encoder.GetByteCount($"{header} ") - encoder.GetByteCount($"{EorzeanScribe.Configuration.ContinuationMarker}");
        ...
        while ( offset < text.Length )
        {
            ...
            if ( str != "\n" && str.Trim().Length > 0 )
                results.Add( new( str.Trim() )
                {
                    ...
                    ContinuationMarker = EorzeanScribe.Configuration.ContinuationMarker
                } );
    ...
    private static string? GetSubstringByByteCount( string text, in int startIndex, in int byteLimit )
    {
        ...
        for ( int length = 1; length + startIndex < text.Length; ++length )
        {
            ...
            if ( text[startIndex + length] == ' ' )
            {
                ...
                if ( EorzeanScribe.Configuration.SentenceTerminators.Contains( text[startIndex + length - 1] ) )
                    lastSentence = length;
            }
            ...
            if ( encoder.GetByteCount( substring ) > byteLimit )
            {
                ...
                if ( EorzeanScribe.Configuration.SplitTextOnSentence && lastSentence > 0 )
