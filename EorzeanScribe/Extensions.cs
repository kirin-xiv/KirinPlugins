...
internal static class Extensions
{
    ...
    internal static string Wrap( this string text, float width, ref int cursorPos )
    {
        ...
        if ( EorzeanScribe.Configuration.ReplaceDoubleSpaces )
    ...
    internal static List<Word> Words( this string s )
    {
        ...
        while ( start + len <= s.Length )
        {
            ...
            if ( start + len == s.Length || " \r\n".Contains( s[start + len] ) )
            {
                ...
                while ( start + wordoffset < s.Length && EorzeanScribe.Configuration.PunctuationCleaningList.Contains( s[start + wordoffset] ) && wordoffset <= len )
                    ++wordoffset;
                ...
                while ( start + len - wordlenoffset - 1 > -1 && $"-{EorzeanScribe.Configuration.PunctuationCleaningList}".Contains( s[start + len - wordlenoffset - 1] ) && wordoffset <= len )
                {
